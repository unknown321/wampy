...
namespace ImGui {
    bool MyImageButtonEx(const char *id, ImTextureID texture_id, ImTextureID pressed_tid, const ImVec2 &image_size,
...
//                    ImClamp(0.0f, 0.0f, g.Style.FrameRounding));
    ...
    bool MySliderBehaviorT(const ImRect &bb, ImGuiID id, ImGuiDataType data_type, TYPE *v, const TYPE v_min,
        ...
        const auto v_range_f = (float) (v_min < v_max ? v_max - v_min : v_min -
...
//        printf("usable max %f, %f\n", slider_usable_pos_max, grab_sz);
            ...
            if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
                ...
                if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
                                                                                     ...
                                                                                     0.0f)) // This is to avoid applying the saturation when already past the limits
                    {
                        ...
                        TYPE v_new = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, clicked_t, v_min,
    ...
    bool MySliderScalar(const char *label, ImGuiDataType data_type, void *p_data, const void *p_min, const void *p_max,
                        const char *format, ImGuiSliderFlags flags,
                        const SliderBarTextures &background,
                        ButtonTextures button, ImVec2 &uv0, ImVec2 &uv1,
        ...
        if (temp_input_is_active) {
            ...
            return TempInputScalar(total_bb, id, label, data_type, p_data, format, is_clamp_input ? p_min : nullptr,
        ...
        // Slider behavior
        ImRect grab_bb;
        const bool value_changed = MySliderBehaviorT<ImS32, ImS32, float>(total_bb,
                                                                          id,
                                                                          ImGuiDataType_S32,
                                                                          (ImS32 *) p_data,
                                                                          *(const ImS8 *) p_min,
        ...
        if (grab_bb.Max.x > grab_bb.Min.x) {
            if (g.ActiveId == id) {
                window->DrawList->AddImage(button.at(0).pressed, grab_bb.Min, grab_bb.Max, uv0, uv1,
                                           GetColorU32(tint_col));
            } else {
                window->DrawList->AddImage(button.at(0).active, grab_bb.Min, grab_bb.Max, uv0, uv1,
                                           GetColorU32(tint_col));
            }
        }
